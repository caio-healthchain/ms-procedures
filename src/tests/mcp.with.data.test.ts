...
/**
 * Suite de Testes com Data Correta dos Procedimentos
 * Testa todos os casos de uso com OpenAI usando 18/12/2025
 */
...
const testDate = new Date('2025-12-18'); // Data dos procedimentos
...
async function testTopProcedures() {
  try {
    ...
    const topProcs = await analyticsService.getTopProcedures('month', testDate, 10, hospitalId);
    ...
    results.push({
      ...
      data: { count: topProcs.length, samples: topProcs.slice(0, 3) },
    });

    console.log(`   Encontrados: ${topProcs.length} procedimentos`);
    if (topProcs.length > 0) {
      topProcs.slice(0, 3).forEach((proc, idx) => {
        console.log(`   ${idx + 1}. ${proc.name} (${proc.total_realizados} vezes, R$ ${proc.valor_total.toFixed(2)})`);
      });
    }
  } catch (error: any) {
    results.push({
...
async function testProceduresStatistics() {
  try {
    ...
    const stats = await analyticsService.getProcedureStats('month', testDate, hospitalId);
    ...
    console.log(`   Valor médio: R$ ${stats.valor_medio.toFixed(2)}`);
  } catch (error: any) {
    results.push({
      name: 'get_procedures_statistics',
      passed: false,
...
async function testEfficiencyMetrics() {
  try {
    ...
    const efficiency = await analyticsService.getEfficiencyMetrics('month', testDate, hospitalId);
...
async function testCategoryAnalysis() {
  try {
    ...
    console.log(`   Duração total: ${analysis.duracao_total} minutos`);
...
${JSON.stringify(results.map(r => ({ 
  ...
  data: r.data ? {
    ...
    valor_total: r.data.valor_total
  } : null
})), null, 2)}
...
async function runAllTests() {
  console.log('🧪 SUITE DE TESTES COM DATA CORRETA - MCP ms-procedures');
  console.log('═'.repeat(80));
